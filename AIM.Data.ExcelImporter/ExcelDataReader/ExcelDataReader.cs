...
namespace Aim.Data.ExcelImporter.ExcelDataReader
{
    ...
    /// </summary>
    public class ExcelDataReader : IDataSetProvider
    {
        ...
        private readonly bool _isRoundingEnabled;
        ...
        public ExcelDataReader(Stream file, bool isRoundingEnabled)
            : this(file, isRoundingEnabled, 2)
        { }

        public ExcelDataReader(Stream file, bool isRoundingEnabled, int roundingDigits)
        {
            m_file = file; // new BufferedStream(file);

            _isRoundingEnabled = isRoundingEnabled;
        ...
        private bool ReadWorksheet(XlsWorksheet sheet)
        {
            ...
            if (idx != null)
            {
                ...
                for (int i = 0; i < dbCellAddrs.Length; i++)
                {
                    ...
                    do
                    {
                        XlsBiffRow row = m_stream.ReadAt(offs) as XlsBiffRow;
                        if (row == null)
                        {
                            break;
                        }
                    ...
                    while (true)
                    {
                        ...
                        switch (cell.ID)
                        {
                            ...
                            case BIFFRECORDTYPE.NUMBER_OLD:
                                dt.Rows[cell.RowIndex][cell.ColumnIndex] =
                                    FormatNumber(((XlsBiffNumberCell)cell).Value);
                                break;
        ...
        private string FormatNumber(double x)
        {
            ...
            {
                return Math.Round(x, _roundingDigits).ToString(CultureInfo.InvariantCulture);
            }
        }
    }
}
