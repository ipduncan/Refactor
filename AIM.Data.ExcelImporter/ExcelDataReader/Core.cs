...
namespace Aim.Data.ExcelImporter.ExcelDataReader
{
    ...
    internal class XlsBiffDimensions : XlsBiffRecord
    {
        ...
        public uint LastRow
        {
            get { return (isV8) ? ReadUInt32(0x4) : ReadUInt16(0x2); }
        }
        ...
        {
            get { return (isV8) ? ReadUInt16(0xA) : ReadUInt16(0x6); }
        }
    }
            ...
            while (offset < last)
            {
                ...
                if (offset + size > last)
                {
                    ...
                    if (encoding == 0 && str.IsMultiByte)
                    {
                        ...
                        string temp = Encoding.Default.GetString(
                            m_bytes,
                            (int)contoffset + 5,
                            (int)len);
                    ...
                    {
                        Buffer.BlockCopy(
                            m_bytes, (int)contoffset + 5, buff, (int)(last - offset), (int)(size - last + offset));
    ...
    internal class XlsFormattedUnicodeString
    {
        ...
        public uint ExtendedStringSize
        {
            get
            {
                return
                    (HasExtString)
                        ? (uint)BitConverter.ToUInt16(m_bytes, (int)m_offset + ((HasFormatting) ? 5 : 3))
                        : 0;
            }
        }
    ...
    internal class XlsBiffLabelCell : XlsBiffBlankCell
    {
        ...
        public string Value
        {
            get { return m_UseEncoding.GetString(ReadArray(0x8, Length * ((m_UseEncoding.IsSingleByte) ? 1 : 2))); }
        }
    }
    ...
    internal class XlsBiffFormulaString : XlsBiffRecord
    {
        ...
        public string Value
        {
            ...
            {
                return
                    m_UseEncoding.GetString(
                        m_bytes,
                        m_readoffset + ((m_UseEncoding.IsSingleByte) ? 2 : 3),
                    ...
                    switch (type)
                    {
                        case 0: // String
                            XlsBiffFormulaString str =
                                GetRecord(m_bytes, (uint)(Offset + Size)) as XlsBiffFormulaString;
                            if (str == null)
                            {
                                return string.Empty;
                            }
            ...
            get
            {
                ...
                if (isV8)
                {
                    ...
                    else
                    {
                        return
                            m_UseEncoding.GetString(
                                m_bytes, m_readoffset + start, (m_UseEncoding.IsSingleByte) ? len : len * 2);
                    }
                }
                else
                {
